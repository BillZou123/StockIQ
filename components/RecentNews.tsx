...
export default function RecentNews({ symbol }: { symbol: string }) {
  ...
  return (
    <div className="space-y-3">
      {news.length === 0 ? (
        <div className="py-16 text-center text-gray-400 text-sm">{t.noNews}</div>
      ) : (
        news.map((item) => (
          <a
            key={item.uuid}
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1.5">
                  <span className="text-xs font-medium text-gray-500">{item.publisher}</span>
                  <span className="text-xs text-gray-300">·</span>
                  <span className="text-xs text-gray-400">{formatNewsDate(item.providerPublishTime)}</span>
                  <ExternalLink className="w-3 h-3 text-gray-300 ml-auto flex-shrink-0 group-hover:text-blue-400 transition-colors" />
                </div>
              </div>
            </div>
          </a>
      ...
      <p className="text-xs text-gray-400 text-right">{t.dataSourceNews}</p>
    </div>
  );
}
